...
// when the data buffer is full the function returns true to inform the user of ADS1256 about reading the data.
bool ADS1256::collectData(void){  
  ...
  _data[currentChannel] = 2*VREF*(((float)data/8388607.0)-1);//Voltage
...
float ADS1256::ReadData(uint8_t channel) {
