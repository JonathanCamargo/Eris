...
FilterOnePole::FilterOnePole( FILTER_TYPE ft, float fc, float initialValue,float constantSPS) {
  setFilter( ft, fc, initialValue,constantSPS);
}

void FilterOnePole::setFilter( FILTER_TYPE ft, float fc, float initialValue,float constantSPS) {
  FT = ft;
  setFrequency( fc );

  if (constantSPS<0){
     isConstantElapsedUS=false;
  }
  else{
     ElapsedUS=(1.0/constantSPS)*1e6;
     TauSamps = TauUS / ElapsedUS;
     ampFactor = expf( -1.0 / TauSamps );
     isConstantElapsedUS=true;
  }
