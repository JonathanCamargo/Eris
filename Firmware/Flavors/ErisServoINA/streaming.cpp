...
bool AddFunction(char * arg){
  ...
  else if (!strncmp("INA",arg,MAXSTRCMP)){
    #if DEBUG
      Serial.println("INA219 selected");
    #endif
    streamfnc[Nfunctions]=&INA219Current;
    Nfunctions=Nfunctions+1;
  }
  else {
    return false;
...
void INA219Current(){
    StreamSamples<inaSample_t,TXBUFFERSIZE>(INA219::buffer,packet);
}
