...
namespace SerialCom{
  ...
  ERIS_THREAD_WA(waReadSerial_T, 4096);
  ...
  //To process //Serial commands
  ERIS_THREAD_WA(waStreamSerial_T, 4096);
  ERIS_THREAD_FUNC(StreamSerial_T) {    
    while(1){
      if (stream_en){
        stream();  
      }                        
      eris_sleep_ms(STREAMING_PERIOD_MS);
    }
  }
  ...
  /***************************************************************/
    ...
    streamSerial=eris_thread_create(waStreamSerial_T, sizeof(waStreamSerial_T),ERIS_NORMAL_PRIORITY, StreamSerial_T, NULL);
