...
public:
  ...
  long droppedCounter; //Number of samples dropped
  ...
  int FetchData(DataType* dest,char * text,uint8_t numSamples); // Request nsamples of data in the buffer
};
...
template <typename DataType>
int ErisBuffer<DataType>::FetchData(DataType* dest,char text[],uint8_t numSamples){  
